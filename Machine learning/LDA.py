...
features_base = np.asarray(np.zeros(50), dtype = "float")
features_stress = np.asarray(np.zeros(50), dtype = "float")
...
for i in range(len(subject)):
    ...
    print(eda_features_stress.shape, emg_features_stress.shape, temp_features_stress.shape,ecg_features_stress.shape)

    features_stress = np.vstack((features_stress, np.hstack((eda_features_stress, emg_features_stress, temp_features_stress, ecg_features_stress))  ))
    features_base = np.vstack((features_base, np.hstack((eda_features_base, emg_features_base, temp_features_base, ecg_features_base)) ))
